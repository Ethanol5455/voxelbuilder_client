...
VertexArray::VertexArray(uint32_t verticiesSize,
			 ...
			 const uint32_t *indicies, uint32_t indiciesCount)
{
	...
	// Index Buffer
	VB_ASSERT(sizeof(uint32_t) == sizeof(GLuint),
		  "Size of uint32_t != GLuint");

	glGenBuffers(1, &m_IBRendererID);
	glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, m_IBRendererID);
	glBufferData(GL_ELEMENT_ARRAY_BUFFER, indiciesCount * sizeof(uint32_t),
...
VertexArray::VertexArray(const void *verticies, uint32_t verticiesSize,
			 const VertexBufferLayout &layout,
			 const uint32_t *indicies, uint32_t indiciesCount)
