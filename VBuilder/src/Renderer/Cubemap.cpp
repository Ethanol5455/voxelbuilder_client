...
Cubemap::Cubemap(const std::string& faces)
{
    ...
    if (data)
    {
        ...
        if (m_BPP == 4)
            format = GL_RGBA;
        glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB8, m_Width, m_Height, 0, format, GL_UNSIGNED_BYTE, data);

        glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, texID, 0);

        VB_ASSERT(glCheckFramebufferStatus(GL_FRAMEBUFFER) == GL_FRAMEBUFFER_COMPLETE, "Cubemap framebuffer not complete");
        ...
        glCopyTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X,
            0, GL_RGB, xStep * 2, yStep * 1, xStep, yStep, 0); // Right Face

        glCopyTexImage2D(GL_TEXTURE_CUBE_MAP_NEGATIVE_X,
            0, GL_RGB, xStep * 0, yStep * 1, xStep, yStep, 0); // Left Face

        glCopyTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_Y,
            0, GL_RGB, xStep * 1, yStep * 0, xStep, yStep, 0); // Top Face

        glCopyTexImage2D(GL_TEXTURE_CUBE_MAP_NEGATIVE_Y,
            0, GL_RGB, xStep * 1, yStep * 2, xStep, yStep, 0); // Bottom Face

        glCopyTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_Z,
            0, GL_RGB, xStep * 1, yStep * 1, xStep, yStep, 0); // Front Face

        glCopyTexImage2D(GL_TEXTURE_CUBE_MAP_NEGATIVE_Z,
            0, GL_RGB, xStep * 3, yStep * 1, xStep, yStep, 0); // Back Face
    ...
    {
        VB_ERROR("Unable to load image: \"{0}\" for reason: {1}", faces, stbi_failure_reason());
    }
    ...
    : m_FilePaths(faces)
{
    ...
    {
        data = stbi_load(faces[i].c_str(), &m_Width, &m_Height, &m_BPP, 0);
        if (data)
        {
