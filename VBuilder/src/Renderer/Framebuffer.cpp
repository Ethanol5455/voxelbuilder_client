...
void Framebuffer::Invalidate()
{
	...
	if (m_ColorAttachmentSpecifications.size()) {
		m_ColorAttachments.resize(
			m_ColorAttachmentSpecifications.size());
	...
	if (m_ColorAttachments.size() > 1) {
		VB_ASSERT(m_ColorAttachments.size() <= 4,
			  "More than 4 FB color attachments not supported!");
...
{
	if (width == 0 || height == 0) {
		VB_WARN("Attempted to rezize framebuffer to {0}, {1}", width,
			height);
		return;
	}
...
void Framebuffer::ClearAttachment(uint32_t attachmentIndex, int value)
{
	...
	glClearTexImage(m_ColorAttachments[attachmentIndex], 0,
			Utils::GFTextureFormatToGL(spec.TextureFormat), GL_INT,
			&value);
}
