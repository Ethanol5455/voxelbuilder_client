...
namespace Utils
{
	...
	static GLenum GFTextureFormatToGL(FramebufferTextureFormat format)
	{
		switch (format)
		{
		...
		VB_ASSERT(false, "Unknown FB Tex format");
		return 0;
	}
}
...
void Framebuffer::Invalidate()
{
	...
	{
		VB_ASSERT(m_ColorAttachments.size() <= 4, "More than 4 FB color attachments not supported!");
	...
	VB_ASSERT(glCheckFramebufferStatus(GL_FRAMEBUFFER) == GL_FRAMEBUFFER_COMPLETE, "Framebuffer is incomplete!");
...
void Framebuffer::Resize(uint32_t width, uint32_t height)
{
	if (width == 0 || height == 0)
	{
		VB_WARN("Attempted to rezize framebuffer to {0}, {1}", width, height);
		return;
	}
...
{
	VB_ASSERT(attachmentIndex < m_ColorAttachments.size(), "");
...
void Framebuffer::ClearAttachment(uint32_t attachmentIndex, int value)
{
	...
	glClearTexImage(m_ColorAttachments[attachmentIndex], 0, 
		Utils::GFTextureFormatToGL(spec.TextureFormat),GL_INT, &value);
}
