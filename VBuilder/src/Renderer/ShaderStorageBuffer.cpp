...
ShaderStorageBuffer::ShaderStorageBuffer(uint32_t dataSize, BufferUsage usage)
{
	...
	glBufferData(GL_SHADER_STORAGE_BUFFER, dataSize, nullptr, Utils::GFBufferUsageToGL(usage));
}

ShaderStorageBuffer::ShaderStorageBuffer(const void* data, uint32_t dataSize, BufferUsage usage)
{
	glGenBuffers(1, &m_RendererID);
	glBindBuffer(GL_SHADER_STORAGE_BUFFER, m_RendererID);
	glBufferData(GL_SHADER_STORAGE_BUFFER, dataSize, data, Utils::GFBufferUsageToGL(usage));
}

void* ShaderStorageBuffer::MapBuffer(BufferAccess access)
{
	return glMapBuffer(GL_SHADER_STORAGE_BUFFER, Utils::GFBufferAccessToGL(access));
}
