...
class Texture {
	...
	void BindImageTexture(bool allowRead, bool allowWrite,
	...
	static TextureCoordinates ExtractCoordinates(const glm::vec2 &count,
						     const glm::vec2 &position);

    private:
	bool LoadTexture(std::string path, uint32_t width, uint32_t height,
			 const std::string &type = "");
};