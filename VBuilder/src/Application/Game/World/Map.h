...
public:
	...
	void FetchColumnFromServer(ENetPeer* server, int columnXPos, int columnZPos);
	...
	void Update(ENetPeer* server, const glm::vec3& cameraPosition, float renderDistance);
	void Render();
	...
	static glm::ivec3 ColumnToWorldPosition(const glm::ivec2& columnPosition, const glm::ivec3& blockPosition);
	static glm::ivec3 WorldToPositionInColumn(const glm::ivec3& worldPosition);
	static glm::ivec3 WorldToPositionInChunk(const glm::ivec3& worldPosition);
