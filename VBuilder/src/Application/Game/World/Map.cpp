...
void Map::FetchColumnFromServer(ENetPeer* server, int columnXPos, int columnZPos)
{
...
void Map::IntegrateColumnFromServer(uint8_t* data, size_t dataLength)
{
	...
	for (int i = 1; i < dataLength; i++)
	{
		...
		ChunkColumn* column = GetChunkColumnAt(columnPos.x, columnPos.z);
		if (column == nullptr)
		{
			std::shared_ptr<ChunkColumn> thisColumn = std::make_shared<ChunkColumn>(glm::ivec2(columnPos.x, columnPos.z));
			m_ColumnMap[columnPos.x][columnPos.z] = thisColumn;
			column = thisColumn.get();
		}

		std::shared_ptr<std::vector<CompressedSet>> compressedData = std::make_shared<std::vector<CompressedSet>>();
		...
		while (BytesToNum<int32_t>(data + i) != -1)
		{
			...
			i += 4;
			compressedData->emplace_back(CompressedSet{ type, number });
		}
		...
		for (int a = 0; a < compressedData->size(); a++)
		{
			...
			for (int q = 0; q < num; q++)
			{
				chunk->SetBlock(index, compressedData->at(a).type);
				index++;
			}
		}
	}
...
void Map::Update(ENetPeer* server, const glm::vec3& cameraPosition, float renderDistance)
{
	glm::ivec2 cameraColumnPos = WorldToColumnPosition(glm::ivec2(cameraPosition.x, cameraPosition.z));
	{
		...
		while (doGenerate)
		{
			ChunkColumn* column = GetChunkColumnAt(columnPos.x, columnPos.y);
			if (column == nullptr)
			{
				FetchColumnFromServer(server, (int)columnPos.x, (int)columnPos.y);
				doGenerate = false;
			}
			else
			{
				glm::vec2 positionDifference = glm::vec2(cameraPosition.x - columnPos.x * 16.0f + 8.0f, cameraPosition.z - columnPos.y * 16.0f + 8.0f);
				float distanceToChunk = (float)glm::sqrt(positionDifference.x * positionDifference.x + positionDifference.y * positionDifference.y);
				if (spiralRadius >= spiralTarget || distanceToChunk > renderDistance * 1.1f)
				{
					doGenerate = false;
					continue;
				}
				...
				if (diff.x == spiralRadius && columnPos.y == cameraColumnPos.y)
				{
				...
				if (diff.y == -spiralRadius && diff.x == spiralRadius)
				{
	...
	for (auto& columnX : m_ColumnMap)
	{
		...
		{
			glm::vec2 positionDifference = glm::vec2(cameraPosition.x - columnX.first * 16.0f + 8.0f, cameraPosition.z - columnZ.first * 16.0f + 8.0f);
			float distanceToChunk = (float)glm::sqrt(positionDifference.x * positionDifference.x + positionDifference.y * positionDifference.y);
			if (distanceToChunk <= renderDistance * 1.2f)
			{
				columnZ.second->SetDoDraw(distanceToChunk <= renderDistance);
			}
			else
			{
				columnZ.second->SetDoDraw(false);
				const auto& it = columnX.second.find(columnZ.first);
...
int Map::GetBlock(glm::ivec3 position)
{
	...
	auto* column = GetChunkColumnAt((int)chunkPosition.x, (int)chunkPosition.z);
	if (column)
	{
		if (chunkPosition.y >= 0 && chunkPosition.y <= 15)
		{
			auto* chunk = &*column->GetChunks()[(int)chunkPosition.y];
			if (chunk != nullptr)
			{
...
void Map::SetBlock(glm::ivec3 position, int type, bool buildMeshes)
{
	...
	glm::ivec2 columnPosition = WorldToColumnPosition(glm::ivec2(position.x, position.z));
	glm::ivec3 blockPositionInColumn = WorldToPositionInColumn(position);

	auto* column = GetChunkColumnAt((int)columnPosition.x, (int)columnPosition.y);
	if (column)
	{
		column->SetBlock(blockPositionInColumn, type);

		if (buildMeshes)
			BuildSurroundingMeshes(columnPosition.x, columnPosition.y);
	}
	else
	{
		//m_ToPlace.push_back({
...
{
	if (m_ColumnMap.count(columnXPos) && m_ColumnMap[columnXPos].count(columnZPos))
...
glm::ivec3 Map::ColumnToWorldPosition(const glm::ivec2& columnPosition, const glm::ivec3& blockPosition)
{
...
{
	glm::ivec3 blockPositionInChunk = glm::ivec3(worldPosition.x % 16, worldPosition.y, worldPosition.z % 16);
...
{
	glm::ivec3 blockPositionInChunk = glm::ivec3(worldPosition.x % 16, worldPosition.y % 16, worldPosition.z % 16);
	if (blockPositionInChunk.x < 0)
	{
		blockPositionInChunk.x = 16 + blockPositionInChunk.x;
	}
