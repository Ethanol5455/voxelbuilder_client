...
void Chunk::SetNeighborChunks(Chunk *top, Chunk *bottom, Chunk *left,
...
{
	m_Blocks[xyzToI((uint32_t)position.x, (uint32_t)position.y,
			(uint32_t)position.z)] = type;
	m_IsMeshUpToDate = false;
}
...
{
	return m_Blocks[xyzToI((uint32_t)position.x, (uint32_t)position.y,
			       (uint32_t)position.z)];
}
	...
	for (int z = 0; z < 16; z++) {
		for (int y = 0; y < 16; y++) {
			for (int x = 0; x < 16; x++) {
				...
				const ItemInfo *thisBlockInfo =
					ItemInfoManager::GetItemByID(
						thisBlockID);
				if (thisBlockID == s_AirID ||
				    thisBlockID == -1) {
					continue;
				} else // Block is not air:
				{
					if (thisBlockInfo->Type ==
					    ItemType::BlockCube) // Standard cube pattern
					{
						...
						if (z == 15 &&
						    m_NeighborChunks
							    [Directions::Right]) {
							drawRightFace = IsTransparent(
								m_NeighborChunks[Directions::
											 Right]
									->GetBlock(
										{ x,
										  y,
					...
					} else if (thisBlockInfo->Type ==
						   ItemType::BlockCross) // Block is a cross pattern
					{
						{
							QuadVertex bl;
							bl.Position = {
...
{
	if (id == s_AirID ||
	    ItemInfoManager::GetItemByID(id)->Type == ItemType::BlockCross ||
	    ItemInfoManager::GetItemByID(id)->IsTransparent)
		return true;
	return false;
}
