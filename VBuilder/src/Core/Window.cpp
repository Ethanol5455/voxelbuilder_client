...
void Window::Create(const std::string &title, uint32_t width, uint32_t height)
{
	...
	m_Window = glfwCreateWindow(width, height, title.c_str(), nullptr,
				    nullptr);

	VB_ASSERT(m_Window, "Unable to create window!");

	glfwGetMonitorContentScale(glfwGetPrimaryMonitor(), &m_Data.xScale,
				   &m_Data.yScale);
...
void Window::SetFullscreen(bool enabled)
{
	if (enabled) {
		glfwSetWindowMonitor(m_Window, glfwGetPrimaryMonitor(), 0, 0,
				     m_Data.width, m_Data.height, 60);
	} else {
		int x, y;
		glfwGetWindowPos(m_Window, &x, &y);
		glfwSetWindowMonitor(m_Window, nullptr, x, y, m_Data.width,
				     m_Data.height, 60);
	}
